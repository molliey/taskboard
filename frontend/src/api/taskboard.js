const API_BASE_URL = (import.meta && import.meta.env && import.meta.env.VITE_API_BASE_URL) || 'http://localhost:8000/api';
...
export const projectAPI = {
  ...
  async getMembers(projectId) {
    try {
      return await apiRequest(`${API_BASE_URL}/projects/${projectId}/members`);
    } catch (error) {
  ...
  // Add member by email
  async addMemberByEmail(projectId, data) {
    try {
      const response = await apiRequest(`${API_BASE_URL}/projects/${projectId}/members/by-email`, {
        method: 'POST',
  ...
  // Get project board (columns with tasks)
  async getBoard(projectId) {
    try {
      const data = await apiRequest(`${API_BASE_URL}/projects/${projectId}/board`);
      return data?.columns || {};
    } catch (error) {
      console.error(`Error fetching board for project ${projectId}:`, error);
      return {};
    }
  },
  ...
  async createTask(projectId, taskData) {
    try {
      const response = await apiRequest(`${API_BASE_URL}/projects/${projectId}/tasks`, {
        method: 'POST',
        body: JSON.stringify(taskData),
      });
      console.log('Task created successfully:', response);
  ...
  async updateTask(projectId, taskId, updates) {
    try {
      const response = await apiRequest(`${API_BASE_URL}/projects/${projectId}/tasks/${taskId}`, {
        method: 'PUT',
        body: JSON.stringify(updates),
      });
      console.log('Task updated successfully:', response);
  ...
  async moveTask(projectId, taskId, moveData) {
    try {
      const response = await apiRequest(`${API_BASE_URL}/projects/${projectId}/tasks/${taskId}/move`, {
        method: 'PUT',
        body: JSON.stringify(moveData),
      });
      console.log('Task moved successfully:', response);
  ...
  async deleteTask(projectId, taskId) {
    try {
      await apiRequest(`${API_BASE_URL}/projects/${projectId}/tasks/${taskId}`, {
        method: 'DELETE',
      });
      console.log(`Task ${taskId} deleted successfully`);
      return true;
